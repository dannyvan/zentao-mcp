...
**日期**：2026-09-09
**状态**：待实施
**范围**：把内网 `vaneland/zentao-mcp-server` 的 Bug 能力拷成公开包，可用 `npx` 安装。不扩需求 / 任务 / 项目 / 附件。

## 背景

内网仓直连禅道 REST API v1，MCP + CLI 双门面，写操作默认 dry-run。装法是 clone 后 `node dist/index.js`，不能 `npx`。
...
| 项 | 值 |
|---|---|
| 做法 | 新建干净仓，不带 GitLab 历史 |
| 本机目录 | `/Users/fanxiaokang/Work/Codes/github.com/zentao-mcp`（与 `dsh-*` 同级，不套 `dannyvan/`） |
| GitHub | 公开仓 `dannyvan/zentao-mcp` |
| npm | `@dannyvan/zentao-mcp`，从 `0.1.0` 起 |
| 启动 | `npx -y @dannyvan/zentao-mcp` |
| 功能 | 与内网仓相同的 Bug 读 / 写；写默认 dry-run |
| 内网仓 | 不动 |
...
1. **公开仓与 npm 包不得出现内网主机名、GitLab 路径、默认实例 URL。** 判据：源码 / README / 注释 / package 字段里搜不到 `vaneland`。拷完后用下面命令验，命中即未完成。
...
4. **不改** `gitlab.vaneland.com/vaneland/zentao-mcp-server`。那边继续给内网用，默认实例可以留。
...
- `test-smoke.mjs` / `test-mcp.mjs`（去掉任何内网地址；没凭证则跳过真连）
...
发布顺序：本机建目录 → 拷源并消毒 → `npm run build` 能过 → `gh repo create dannyvan/zentao-mcp --public` → 首推 `main` 建仓（空仓没有可审基线，不空转 PR）→ 使用者 `npm login`（本机 2026-09-09 尚未登录）→ `npm publish --access public`。不设 GitHub Actions。
...
本地：`npm run build` 成功；`npx --package . zentao-mcp help`（或 `node dist/index.js help`）打出 CLI 用法且无 `vaneland`。

消毒：

```bash
rg -n -i 'vaneland' /Users/fanxiaokang/Work/Codes/github.com/zentao-mcp --glob '!node_modules/**' --glob '!.git/**'
```

命中即失败。不把「文件个数 / 版本号」写进结论。
...
真连禅道的 smoke 不作为本版必过项：公开包没有内网凭证，也不得把内网凭证写进 CI。
...
- 不把 `dist` 提交进 git
- 不设 CI 自动 publish
- 不改内网仓、不改 dotfiles 的全局 MCP（禅道 MCP 仍按需给项目，不配全局）
